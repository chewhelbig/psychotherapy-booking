...
import { SCHEDULE, getHoursForDate, getBufferForDate } from './schedule';
...
export async function getAvailableSlots(year, month, day, dayOfWeek, sessionType) {
  ...
  // Hours and buffer for this date (rules switch on 22 June 2026).
  ...
  // Slot sizing.
  //   eventMs  = calendar event block (50 + 10 individual, 80 + 10 couples).
  //   bufferMs = required gap between consecutive events.
  const eventDuration = session.eventDuration || (session.duration + 10);
  const eventMs = eventDuration * 60000;
  const bufferMs = bufferMin * 60000;
  const stepMs = eventMs + bufferMs;

  // Build free blocks — continuous time between busy ranges,
  // padded by `bufferMs` on each side of every busy range.
  const freeBlocks = [];
  ...
  // slotStart = the time the client sees (and what gets saved to Stripe / calendar).
  // slotEnd   = the calendar event end (slotStart + eventDuration), which includes the in-event buffer.
  const slots = [];
