...
export async function getAvailableSlots(year, month, day, dayOfWeek, sessionType) {
  ...
  // Look up hours by date (Mondays open + 40-min buffer from 1 July 2026).
  // Use noon SGT to avoid any midnight/DST edge cases.
  const sessionDate = new Date(sgtISO(year, month, day, 12, 0));
  const dayHours = getHoursForDate(sessionDate);
  const buffer = getBufferForDate(sessionDate);
  console.log('SLOTS:', JSON.stringify({ year, month, day, open: !!dayHours, buffer }));
  if (!dayHours) return [];
  ...
  // Generate slots
  const slotDurationMs = (session.duration + buffer) * 60000;
